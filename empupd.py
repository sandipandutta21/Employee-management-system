    ...
    try:
            ...
            if( len(df) == 0):              # If there is no record matching then len of DF would be zero
                ...
                for index,row in df.iterrows():
                    ...
                    empname_txt.config(textvariable=name, state=NORMAL)
                    ...
                    add1.config(textvariable=add_1, state=NORMAL)
                    add2.config(textvariable=add_2, state=NORMAL)
                    add3.config(textvariable=add_3, state=NORMAL)
                    
                    #print(row['pin'])
                    pincode = StringVar()
                    pincode.set(row['pin'])
                    pin.config(textvariable=pincode, state=NORMAL)
                    
                    #print(row['city'])
                    cityname = StringVar()
                    cityname.set(row['city'])
                    empcity_txt.config(textvariable=cityname, state=NORMAL)
                    ...
                    ddmmyy = dd+'/'+mm+'/'+yy
                    birth_dt.set(ddmmyy)                        
                    birth_txt.config(textvariable=birth_dt, state=NORMAL)
                    
                    #print(row['basic'])
                    basic_amt = IntVar()
                    basic_amt.set(row['basic'])
                    basic_txt.config(textvariable=basic_amt, state=NORMAL)
                    
                                      
                    #print(row['conv'])
                    conv_amt = IntVar()
                    conv_amt.set(row['conv'])
                    conv_txt.configure(textvariable=conv_amt, state=NORMAL) 
...
def upd_database():
            ...
            eno = empno_txt.get()
            conn = mysql.connector.connect(user='malay',password='mararane4', host='127.0.0.1', database='testdb')
            # Check if connected
            if( conn.is_connected()):
               # print("Connected to MySQL Database")
            ...
            name = empname_txt.get()
            marital_stat = empmarried_state.get()
            
            gender_type=gender_selected.get()       
            address1 = add1.get()
            address2 = add2.get() 
            address3 = add3.get()         
            pin_str = pin.get()
            cityname = empcity_txt.get()
            
            
            dt = birth_txt.get()
            dob = datetime.strptime(dt,'%d/%m/%Y')     # strptime() parses a time string according to a format.
         
            basic_amt = basic_txt.get()
            conv_amt = conv_txt.get()

            full_add = address1+address2+address3
                       
            sql_str="update empmaster set empname=%s, married_status=%s, gender=%s, address=%s, pin=%s, city=%s, birth_dt=%s, basic=%s, conv=%s where empno=%s"           
            args = (name, marital_stat, gender_type, full_add, pin_str, cityname, dob, basic_amt, conv_amt, eno) 
   
            # Execute the SQL query
            cur.execute(sql_str, args)
                                  
            # save the changes. cur.rowcount gives no. of records affected due to DML statements
            conn.commit()
...
#----------------------------------------------------------------
window = Tk()
window.title("Welcome to Employee Master Update App")
window.geometry('900x500')      # width x height

# Labels to display on screen
lbl = Label(window, text='Update Employee Details', font=('Arial Bold', 10, 'underline'), fg='blue', bg='light grey')
...
upd_btn = Button(window, text='Update Data', bg='Orange', fg='Black', command=upd_database)
upd_btn.grid(column=3, row=15)

ext_btn = Button(window, text='Exit', bg='orange', fg='black', command=window.destroy)
ext_btn.grid(column=4, row=15)
